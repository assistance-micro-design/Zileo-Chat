...
A `workflow_complete` listener inside the scheduler module performs the reverse lookup (`card_id_for_workflow` Tauri command), transitions the linked card from `doing` to `review`, persists the `error_summary` on failure, and — if the target agent has `auto_analyze_reports: true` — fires `analyze_card_report` against the configured Kanban (supervisor) agent. The analyze tool loop forces a tool call on its opening turn (`opening_tool_choice = Required`) so the supervisor must emit its `SubmitAnalysisTool` verdict rather than replying in prose and leaving the card stuck in `review`; subsequent turns revert to `Auto` so the loop can terminate.

A boot-time catch-up pass (`catchup_unanalyzed_review_cards_core`, spawned detached at startup in `main.rs`) re-runs the analyzer for every card stuck in `review` that finished into `done`, has a `workflow_id`, and has no `analyze` interaction yet — covering cards orphaned by an app closed between the worker workflow completing and the listener firing. Best-effort and self-gated by `auto_analyze_reports`. The verdict language follows `workflow.locale` (stamped at execution, read back via `workflow_id`), so the supervisor answers in the UI language without a frontend round-trip.
