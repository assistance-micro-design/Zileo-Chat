...
pub enum KanbanCardStatus {
    ...
    Done,
    Failed,
    /// Generated by an async `start_compose_card` run and awaiting human
    /// validation. Stored with `column='todo'` (placeholder; the column field
    /// is NOT NULL) but NEVER auto-promoted — the scheduler only promotes
    /// `status='ready'`. Validating it flips the status to `Ready`.
    Proposed,
}

impl KanbanCardStatus {
    /// Wire string of the status (matches the serde `rename_all = "lowercase"`
    /// representation and the schema ASSERT). Used to embed a HARDCODED status
    /// value in a `CREATE`/`UPDATE` query — never user-controllable, so safe to
    /// `format!`.
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            KanbanCardStatus::Proposed => "proposed",
        }
    }
}
...
mod tests {
    ...
    fn test_status_proposed_round_trip() {
        let parsed: KanbanCardStatus =
            serde_json::from_str("\"proposed\"").expect("\"proposed\" must deserialize");
