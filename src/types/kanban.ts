 ...
 *
 * `proposed` = generated by an async `start_compose_card` run and awaiting human
 * validation (stored with `column='todo'` but excluded from the board and never
 * auto-promoted by the scheduler — validating it flips it to `ready`).
 */
...
export interface KanbanCardCreate {
	/**
	 * Optional pre-generated card id. Set by `start_compose_card` so the
	 * persisted `kanban_card_interaction` row can be linked to the card
 ...
 * the detached generation task is spawned. The card itself (status `proposed`)
 * arrives later via the `kanban:compose_ready` event.
 */
export interface ComposeStartResponse {
	/** Tracking id, also the future card id (`compose_id == card_id`). */
	card_id: string;
}
