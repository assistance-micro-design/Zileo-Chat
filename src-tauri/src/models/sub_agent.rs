...
pub struct SubAgentExecution {
    ...
    /// (`compute_sub_agent_cost`). Persisted on every completed execution.
    /// `None` for legacy rows that predate the column or when the model has
    /// no pricing entry. Surfaced by `load_workflow_blocks` so replayed
