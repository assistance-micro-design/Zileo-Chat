...
| `KanbanCardReportViewer.svelte` | Read the workflow report for a finished card, inline interaction history (compose + analyze), launch / re-run analyze, force-delete a stuck `doing` card. For cards in `review` **or `done`** it also hosts an in-place **review chat** with the card's supervisor agent (seed / resume via `open_card_review_chat`), shown in a full-screen `Modal` (`fullscreen` prop). The chat reuses the same `ChatContainer` + `WorkflowExecutorService.execute` + `executionBlocksStore` pipeline as the `/agent` page: the supervisor's turn streams live into the modal, persisted tool-call and reasoning blocks are replayed on reopen (`BlockService.loadForMessages` → `load_workflow_blocks`), and the turn keeps running detached on close — reopening mid-run restores the in-flight timeline. `onboardchanged` refreshes the board + card after a tool acts |
...
| `agent-reasoning.ts` | `getReasoningOptions()`, `getReasoningHelp()`, `supportsXhighReasoning()`, `normalizeReasoningEffortForProvider()` | Provider-aware reasoning_effort selector helpers (Mistral exposes Off/High only; other providers expose Off/Low/Medium/High, plus `xhigh` "Think Max" when the model matches `XHIGH_MODEL_PATTERNS`). `normalizeReasoningEffortForProvider` downgrades `xhigh` only when the model is known and unsupported — an unknown model (LLM list still loading on form open) preserves the stored value to avoid clobbering a persisted `xhigh`. |
