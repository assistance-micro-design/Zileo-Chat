...
- The MCP tool allowlist is reset fail-closed on import (R-SEC-4), and imported MCP servers are re-validated through the SSRF/Docker screen — local/LAN HTTP servers export fine but are refused on re-import and flagged up front in the export preview
