...
- **Conversation header shows the model and a reasoning badge** (`src-tauri/src/models/agent.rs`, `src/types/agent.ts`, `src/lib/components/agent/AgentHeader.svelte`, `src/messages/{en,fr}.json`) -- the read-only header above the chat now displays, besides the agent name and max iterations, the agent's model (monospace) and a tinted badge with its configured reasoning effort. `AgentSummary` gains an optional `reasoning_effort` field (Rust `Option` skipped when absent / TS optional, covered by unit tests) so the header reads it from the existing agent list without loading full configs.
- **Switch component for boolean settings** (`src/lib/components/ui/Switch.svelte`, `settings/speech-to-text`) -- "enable X" settings move from bare checkboxes to an accessible on/off switch (native button with `role="switch"` + `aria-checked`, brand-gradient track when on, focus ring). First adopted on the dictation enable toggle; list-selection checkboxes (tool allowlists, export pickers) intentionally stay checkboxes. Adoption continues surface by surface.
- **Unsaved-changes indicator on long settings forms** (`src/lib/utils/dirty.ts`, `AgentForm`, `ValidationSettings`, `AgentAuthorizations`) -- the save actions of long forms become a sticky translucent bar pinned to the bottom of the scroll area, with an "unsaved changes" hint whenever the form values diverge from what they were seeded with. The comparison goes through a new key-order-insensitive snapshot helper (unit-tested) shared by all forms.
- **Configurable global supervisor agents for the Kanban board** (`src-tauri/src/commands/{settings_kanban,compose_card,kanban_analyzer,kanban_card_chat}.rs`, `src/lib/stores/kanban-settings.ts`, `src/lib/utils/kanban-supervisors.ts`, `KanbanCardCreator*`, `KanbanSupervisorNotice.svelte`, `routes/settings/kanban`, `src/messages/{en,fr}.json`) -- the two Kanban supervisor roles (the agent that composes new cards and the agent that analyzes reports) can now each be pinned to a specific agent from the Kanban settings instead of always using the implicit default. The backend `KanbanSettings` gains `compose_agent_id` / `analyze_agent_id` (tri-state, so "unset" and "cleared" stay distinct), resolved through a shared helper that keeps the configured agent only while it still exists and is a Kanban-kind agent, otherwise falling back gracefully; analyze resolves the effective agent before the auto-report gate and substitutes it on every card. A read-only preview command renders the real role system prompt, and the card creator shows an inline notice when a configured role is unset or points at a deleted agent.
- **Keyboard UI zoom** (`src/lib/stores/ui-zoom.ts`, `src/lib/tauri/webview.ts`, `FloatingMenu.svelte`, `routes/+layout.svelte`, `src-tauri/capabilities/default.json`, `src/messages/{en,fr}.json`) -- Ctrl/Cmd `+` / `-` / `0` now zoom the whole interface through the native WebView zoom -- which also scales images and fixed-pixel surfaces, unlike a CSS rem rescale -- clamped between 50% and 200% and persisted in local storage. Because the native zoom is session state Tauri does not restore on restart, it is re-applied on boot; because WebKitGTK ships its own Ctrl +/- handler, the key handler calls `preventDefault` to avoid double application. A pill control in the floating dock (`[ - | 130% | + ]`, click to reset, hidden below 640px) mirrors the shortcuts.

### Changed

- **Visual identity aligned on assistancemicrodesign.net ("Lueur" v2)** (`src/styles/global.css`, 114 token-driven components) -- the palette moves to a light-first design: white and signature-cream surfaces, cool greys, the site's radii and soft wide shadows, brand glows and gradients. New tokens (`--surface-1/2/overlay/cream`, `--color-accent-deep`, `--color-online`, `--color-link`, `--channel-*`, `--glow-*`, `--gradient-brand/rail`) join the existing names -- every existing token and compatibility alias is preserved, so all components reskin from the stylesheet alone. Accent-colored **text** moves to the AA-readable deep accent ink everywhere (38 declarations across 28 components); fills and borders keep the brand turquoise. Stale hex fallbacks in `var()` declarations are stripped; the theme-preview hexes in onboarding are documented as intentional.
- **Floating dock with segmented navigation** (`FloatingMenu.svelte`, `src/styles/global.css`) -- the full-width top bar becomes a detached, translucent, blurred pill centered near the top of the viewport: brand mark with a gradient dot, "new task" button on the brand gradient, and a segmented navigation pill where the active tab carries a glowing surface and its icon takes the page identity color (turquoise tasks, orange agent). If the blur ever proves costly on WebKitGTK, the opaque fallback is a one-token change (`--surface-overlay` alpha).
- **Splash screen restyle** (`SplashScreen.svelte`) -- breathing brand mark, radial turquoise veil (light) / site dark gradient (dark), glowing dots on the boot ticker. Behavior untouched: 900 ms minimum display, fade-out, reduced-motion freeze.
- **Execution thread on color channels with a vertical rail** (`ThinkingBlock`, `ToolCallBlock`, `SubAgentBlock`, `TodoTasksBlock`, `ExecutionSpinner`, `ChatContainer`) -- each block type carries its own color channel (thinking violet, local tool blue, MCP magenta, sub-agent orange, tasks turquoise) on a tinted icon pill, and both block groups (persisted under a message, live during execution) draw a vertical gradient rail with one glowing node per block; the newest live node pulses while streaming, paused during scroll and disabled under reduced motion. Tool failures keep a loud red rib. The tasks block gains a conic completion ring (the textual counter beside it doubles as fallback). The Kanban report viewer inherits the channels through the shared blocks.
- **Composer pill and message bubbles** (`ChatInput.svelte`, `MessageBubble.svelte`, `MessageMetrics.svelte`, `TokenDisplay.svelte`) -- the chat input becomes a rounded pill with a focus glow and a square gradient send button; the user bubble sits on the brand gradient with dark ink (white on pale turquoise failed AA contrast); the token metrics bar moves to a translucent rounded surface with a green-to-turquoise context gauge and the cost on the readable deep orange ink.
- **Kanban board tinted by status** (`KanbanColumn`, `KanbanCardItem`, `routes/kanban`) -- column headers carry a glowing status dot and an outlined counter pill; cards inherit the column tint as a left rib, lift on hover, and a running card shows a pulsing inner halo. Card actions rest at 0.45 opacity and reveal on hover or keyboard focus. The proposed-cards review zone moves to the signature cream surface with a dashed brand border and a soft halo.
- **Settings surfaces on the new design** (`settings/+layout.svelte`, `SettingsSectionHeader`) -- the active section of the settings nav carries a glowing gradient bar, and section headers move to the larger title + width-capped description treatment.
- **Onboarding rebuilt as a two-panel wizard** (`OnboardingModal`, `OnboardingProgress`, `StepFeatures`, `src/messages/{en,fr}.json`) -- a brand panel (cream surface, turquoise veil, vertical stepper with done / active / upcoming states and a "Step x of y" label) sits beside the step content with a constant Previous / Skip / Next footer. Skip is hidden on the language step (the choice applies immediately, making skip redundant with Next). Feature cards take the execution-channel tints. French texts are harmonized: proper ellipsis in "Test en cours…", "Réglages" unified to "Paramètres" (11 keys), "la page Agents" to "la page Agent".
- **Sub-agent execution threads nest inside their summary block** (`chat-container-helpers.ts`, `ChatContainer`, `SubAgentBlock`, `ThinkingBlock`, `ToolCallBlock`, `src-tauri/src/models/chat_block.rs`) -- a sub-agent's internal blocks (tool calls, reasoning) no longer render flat in the main thread: they nest inside the sub-agent's collapsible summary block (metric chips for tokens/cache/thinking, report text, internal thread on a fading orange rail with per-channel nodes), collapsed by default so a long delegation folds into a single line. The flat-thread indent with a dashed orange border is removed -- the agent tag and the nesting carry the attribution. While a sub-agent is still streaming (its summary not emitted yet) its blocks stay flat, then regroup on completion; the replay path projects the sub-agent id onto the summary so persisted workflows nest identically.
- **Agent page aligned on the v2 mockups** (`WorkflowSidebar`, `WorkflowList`, `WorkflowItem`, `FolderItem`, `StatusFilters`, `ChatInput`, `ToolCallBlock`, `MessageBubble`, `MessageMetrics`, `TokenDisplay`, `ValidationModal`, `DeleteConfirmModal`, `NewWorkflowModal`, `routes/agent`, `src/styles/global.css`) -- the workflow sidebar becomes a floating card (secondary surface, border, radius) detached from the viewport edges by a page gutter, with search + help on its first row, outlined filter chips, folder headers rendered as uppercase group labels (no more separators or indent), and the New workflow / New folder / Selection mode actions stacked at the bottom as in the mockups. Active workflow items sit on a plain surface with a soft brand glow instead of a turquoise fill. In the conversation: tool blocks gain Input/Output tabs (failed calls open on the output), timestamp and copy move below the bubbles, assistant metrics become pill chips, the queued-message hint becomes an amber pill above the textarea, keyboard shortcuts stay visible under the composer, and a ghost cancel button sits beside the always-visible send button during execution. The validation modal moves its queue badge to the footer (new neutral badge variant) with a soft-red Reject (new button variant); delete confirmations adopt the amber alert pattern; empty states and the token bar take the mockup spacing and inks.

- **Providers settings page aligned on the v2 mockups** (`LLMSection`, `ProviderCard`, `ModelCard`, `ModelForm`, `CustomProviderForm`, `APIKeysSection`, `ui/Card`, `ui/Modal`, `routes/settings/providers`, `src/styles/global.css`, `src/messages/{en,fr}.json`) -- the page takes a single header (title, description and the primary "Add model" action) instead of two stacked section headers; the Models sub-section becomes a lighter heading with the provider filter beside it. Builtin provider cards condense to icon + name + a checked status badge ("API key configured" / "Server available"), a one-line description, the connection tester and an outlined configure-key action; the Ollama card now edits the server URL in place (validated, saved on blur, with toasts) instead of showing it read-only behind a modal. Custom providers move from full-size cards to compact entity rows (name, base URL with the cache/reasoning toggle summary, edit / test / delete icon actions revealed on hover or keyboard focus) inside a card whose footer carries the outlined "Add provider" button; their API key is managed from the edit form itself (write-only field, blank keeps the stored key), which gains a two-column id/name row, on/off switches for the strict-mode toggles and an inline "Test connection" action reporting through toasts. Model cards lead with the monospace API name, a neutral "Builtin" badge, two condensed spec lines and capability badges (vision, reasoning on the thinking channel); builtin models become editable from the card (the form locks the read-only fields and its notice now lists them), deletion stays custom-only as enforced by the backend. The model form opens in a new wide modal variant with three-column rows, a flattened four-column pricing block and switches instead of checkboxes; its inner title duplicating the modal title is removed. The API key modal moves to a password field with a show/hide eye and anchors "Delete key" left as a soft-destructive action. Cards in both grids take the mockup hover lift through a new opt-in `card-hover` recipe, and 14 i18n keys orphaned by the redesign are removed from both locales.

- **Agents settings page aligned on the v2 mockups** (`AgentList`, `AgentForm`, `AgentFolders`, `ui/Badge`, `ui/Switch`, `src/styles/global.css`, `src/messages/{en,fr}.json`) -- agent cards take the header / body / footer structure with the hover lift: bot icon on the agent channel orange, lifecycle and supervisor badges in the header, stacked icon detail lines (provider, monospace model) where the tool and MCP counts become badges (the MCP count reads on a new magenta `mcp` badge variant when servers are configured), and a footer with the edit action and a soft-destructive delete. The edit form drops its two-column section grid for the mockup's stacked sections with accent-tinted headings: name / lifecycle / agent-kind condense into a three-column row of selects (the lifecycle card picker is gone; its description moves into the help line), provider and model collapse from card pickers to compact selects (monospace API name with a "Builtin" marker -- the spec lines live on the providers page), the system prompt moves up next to the LLM block, and the per-agent grants (tools on a two-column grid, MCP servers, skills) switch from checkboxes to the design-system Switch rows. Authorized folders become entity rows (folder icon, monospace path, icon-only remove) inside a sub-card whose footer carries the outlined "Add folder" button, above a help line stating the validation rules. The sticky save bar keeps its unsaved-changes hint and gains the mockup's alert icon. Three i18n keys orphaned by the redesign are removed from both locales.

- **MCP and Kanban settings pages aligned on the v2 mockups** (`MCPSection`, `MCPServerCard`, `MCPServerForm`, `MCPServerTester`, `routes/settings/mcp`, `routes/settings/kanban`, `src/messages/{en,fr}.json`) -- the network opt-in moves from a bare checkbox to a Switch toggle row inside a width-capped card, with the SSRF warning restyled as a bordered amber alert. Server cards take the header / body / footer structure with the hover lift: server icon on the magenta MCP channel, a status badge carrying a glowing status dot ("Running" reworded to match the running state), a "Disabled" badge plus a dimmed card when the server is off, condensed detail lines (human-readable deployment method with the auth method for HTTP servers, tool and resource counts with an em dash while unknown) replacing the raw command line, and an all-ghost action footer where delete becomes icon-only. The server form opens in the wide modal variant: name (monospace) and deployment method share a two-column row, environment variables and extra headers gain outlined "add" buttons below their rows, the sections reorder to match the mockups (env vars and description before the HTTP auth block, whose method and secret sit side by side), the enable checkbox becomes an "Automatic startup" switch row and the reserved-Authorization-header rule is now stated under the headers editor. The connection tester compacts to a status badge with latency, discovered tools and resources as monospace badges on the MCP channel (descriptions and URIs on hover), errors in a monospace panel, and its retry action moves to the modal footer as an outlined button. The Kanban settings card is width-capped with a compact number field and gains the mockup footer: an outlined Reload and a primary Save button that now applies the timeout explicitly (the save-on-blur behaviour is retired, the contextual help updated accordingly). Three orphaned i18n keys are removed from both locales.

- **Prompt and Skill library pages aligned on the v2 mockups** (`PromptList`, `PromptForm`, `PromptSettings`, `SkillList`, `SkillForm`, `SkillSettings`, `VersionsHistoryModal`, `src/messages/{en,fr}.json`) -- both libraries drop their card grids for compact entity rows inside a single card: book icon on the deep accent, name with inline category badge (plus the target-kind badge for skills), a one-line meta (description, variable count or character count, last update) and icon-only actions (edit, version history, delete) revealed on hover or keyboard focus. Version history is now reachable directly from each row -- it previously hid behind the edit form -- and the history modal itself takes the entity-row treatment: latest version badged on the brand color, edit summary as the row title, date and author beneath, View / Restore / Delete as quiet icon actions (last remaining version still protected) and a proper Close footer. The skill enable toggle moves from a bare checkbox to the design-system Switch on the row (the mockup shows a static state badge, but a badge alone would lose the one-click toggle since the form has no enabled field). The search filters gain the in-field magnifying glass with the category select beside them. Both edit forms leave the fullscreen modal for the wide variant: name and category share a two-column row (skill names monospace), the mockup help lines appear under the name, description, content and variable fields, detected variables and skill references render as monospace badges inline under the content with the skill-reference inserter beside them, and the name/description character counters disappear (content keeps its 50k counter). Four orphaned i18n keys are removed from both locales.

- **Validation settings page aligned on the v2 mockups** (`ValidationSettings`, `AgentAuthorizations`, `AgentMcpAllowlist`, `src/messages/{en,fr}.json`) -- the global validation settings move into a single card with a sticky save bar (unsaved-changes hint, ghost Reset, primary Save) and small accent-tinted group headings. The mode selector takes the mockup card recipe (thin border, deep-accent border with a soft glow and light accent fill when selected), the selective sub-options become toggle rows inside a dashed enclosure with the checkbox on the right (dropping the per-category tool and server badge lists), and the risk-threshold and audit-logging checkboxes become Switch rows. The timeout block trades its slider and radio group for a compact number field beside a behavior select with explicit range help lines; audit retention becomes a compact number field, with the purge action as a small outlined button and the audit-log link as a quiet button with an external-link icon. Per-agent authorizations move into a card too: width-capped agent select with the configured-marker legend as its help line, the MCP auto-approval heading carrying an "Unsaved changes" badge while edits are pending, the unattended-run warning as a bordered amber alert, and the file-confirmation toggle as a Switch row above the sticky save bar (the two scope-explanation paragraphs collapse into the section description). Allowlist servers render as nested cards: collapsible header with the armed count on the MCP badge plus quiet "approve all" / "remove all" actions replacing the in-list select-all checkbox, tools on a responsive monospace checkbox grid, and the delegation flag as a Switch row under the grid; preserved entries from stopped or de-selected servers become dimmed cards with count and status badges and an icon-only revoke. Seven orphaned i18n keys are removed from both locales.

- **Memory settings page aligned on the v2 mockups** (`MemorySettings`, `EmbeddingConfigCard`, `EmbeddingTestCard`, `MemoryForm`, `MemoryList`, `MemoryStatsCard`, `routes/settings/memory`, `src/messages/{en,fr}.json`) -- the embedding configuration is edited in place instead of behind a modal, reindex and purge merge into a single Operations card, and the memory manager becomes one card with a toolbar above the shared global table. The connection test renders in a monospace code panel, memory type badges are localized with realigned variants, and a new short-date helper (unit-tested) formats the list timestamps.

- **Import/Export settings page aligned on the v2 mockups** (`ImportExportSettings`, `ImportExportSteps`, `ExportPanel`, `ImportPanel`, `ConflictResolver`, `MCPEnvEditor`, `src/messages/{en,fr}.json`) -- the import/export tabs become a segmented pill, a new shared `ImportExportSteps` component draws the numbered round/check/gradient stepper for both flows, export options move to Switch toggle rows with an info alert, and import warnings render as amber alerts. The conflict resolver is rebuilt as compact entity rows with a select and grouped action buttons, and the MCP env editor takes the magenta `mcp` badge. Per-item selection, previews and multi-field editing are kept.

- **Audit-log settings page aligned on the v2 mockups** (`AuditLogStats`, `AuditLogFilters`, `AuditLogList`, `AuditLogRow`, `ui/Input`) -- the statistics move to rounded metric chips with tinted icons, the filters condense into a card toolbar of compact inputs and selects (each with an aria-label), the entries render on the shared global table with decision and risk badges, and pagination spreads to the row edges. The shared Input component gains an `ariaLabel` prop for the otherwise unlabeled filter fields.

- **Speech-to-text settings page aligned on the v2 mockups** (`routes/settings/speech-to-text`) -- the inline enable checkbox becomes a toggle row (label left, Switch right) and Save / Reset move into a right-aligned card footer. The card is width-capped, the model field is monospace and the language select width-capped, the "models" link becomes a small ghost button, and the shortcut hint pairs an info icon with tertiary helper text. Behavior and i18n are unchanged.

- **Theme settings page aligned on the v2 mockups** (`routes/settings/theme`, `src/messages/{en,fr}.json`) -- selected theme cards take the deep-accent border with a soft glow, the light/dark previews are tinted, the page gains a header description, and the security note condenses into a compact card; the blocks are width-capped.

- **Settings sidebar floated with a live provider status footer** (`routes/settings/+layout.svelte`, `src/messages/{en,fr}.json`) -- the settings navigation becomes a floating card aligned with the Agent page through a page gutter, and its footer swaps the static "AES-256 encryption" badge for a live Providers panel: a glowing dot per provider state (not configured / checking / available / offline), tested in parallel at the single layout mount through the 30 s LLM cache, collapsing to a stack of dots with name/state tooltips when the sidebar is folded, with the pulse disabled under reduced motion.

- **Kanban board page polished against the v2 mockups** (`KanbanCardItem`, `KanbanCardCreator`, `KanbanCardEditModal`, `KanbanCardReportViewer`, `KanbanFiltres`, `KanbanScheduleForm`, `KanbanScheduleModal`, `KanbanImprovePromptModal`, `ui/Button`, `ui/Select`, `routes/kanban`, `src/styles/global.css`) -- the board adopts outline buttons, a segmented card creator and across-the-board modal polish, with the shared Button and Select components extended to carry the new variants.

- **Always-visible library row actions and a prompt content toolbar** (`PromptList`, `SkillList`, `PromptForm`, `src/messages/{en,fr}.json`) -- the prompt and skill entity-row actions are always visible instead of revealed on hover or focus, and the prompt form's content meta is rebuilt as a toolbar (skill select plus a `.../50,000` character counter) with detected variables and skill references moved into a chip section carrying per-kind counters.

- **Animations consolidated on the design tokens** (12 components) -- component-local `@keyframes` duplicating the global `spin` / `fadeIn` / `pulse` definitions are removed (the animation names resolve to the shared keyframes, eliminating drift such as 8px vs 10px fade offsets), and hardcoded entry-animation durations (0.15s/0.2s/0.3s mix) now use the `--transition-fast/base` scale. Intentionally distinct effects (opacity-only backdrop fades, the modal slide-up with scale, the mic button pulse ring) stay local; the execution spinner's opacity-only fade is renamed `fadeInOpacity` so it no longer shares a name with the global sliding `fadeIn`.
- **Font sizes aligned on the typographic scale** (kanban subsystem, `VersionsHistoryModal`, chat/settings labels) -- ad-hoc sizes between the scale steps (0.7 to 0.875rem and 10px) are mapped onto the existing tokens, with a new `--font-size-2xs` (0.625rem) reserved for micro-labels (REC badge, filter counters, token display). The remaining hardcoded heading and title sizes (0.9 / 0.95 / 1.05 / 1.4 / 1.5rem in the kanban board, schedule modal, report viewer and versions history modal) are snapped to the nearest scale step, preserving the title-over-body hierarchy in each context.
...
- **Settings section headers unified on the shared component** (`settings/kanban`, `settings/mcp`, `settings/speech-to-text`, `settings/+layout.svelte`) -- three pages built their headers by hand with an `h2` at the page-title size (`2xl`), while every other section renders an `h3` at the section size (`lg`) through `SettingsSectionHeader`; on the MCP page the two patterns even coexisted on screen (network section `h2` above the servers section `h3`), breaking the heading hierarchy. All three pages now use the shared header, which also gives them the contextual help button (with new EN/FR help content) that every other section already had, and removes their duplicated local title/description CSS. The page-level `.section-title` / `.section-title-row` rules in the settings layout no longer had any page consumer and leaked a `2xl` font size and stray margins into component-scoped classes of the same name; they are removed.
- **Speech-to-text i18n keys renamed from `stt.*` to `stt_*`** (`src/messages/{en,fr}.json`, `MicButton.svelte`, `sttStore.svelte.ts`, `settings/speech-to-text/+page.svelte`) -- the 35 dictation UI keys were the only settings keys using dot-separated names; they now follow the underscore convention used by every other settings key. The `errors.*` namespace is intentionally left as is (it mirrors backend error codes).

### Fixed

- **Slow scrolling on the settings pages** (`src/styles/global.css`, `settings/+layout.svelte`, `AgentForm`, `ValidationSettings`, `AgentAuthorizations`) -- three compounding paint costs made every settings page scroll noticeably slower than the chat, kanban or modal surfaces, all of which scroll inside opaque containers. The settings content area was transparent over the body veil gradient, so WebKitGTK could not blit the moving pixels and repainted the whole visible area (dense with shadowed cards) on every frame; it now carries an opaque background. The body gradient also used `background-attachment: fixed`, a property documented to force per-scroll repaints and disable compositing optimizations -- it is removed with zero visual change since the app shell never scrolls the body. Finally, the sticky save bars sat on a translucent blurred surface (`backdrop-filter`), forcing a re-blur of the content scrolling behind them on every frame; they move to an opaque surface. The settings scroller also gains `will-change: scroll-position` -- unlike the chat thread, whose animated rail nodes force composited layers and pull its scroller onto the accelerated path, settings pages have no composited descendant and otherwise stay on main-thread scrolling. Worst of all, mouse-wheel scrolling advanced one notch at a time and then stalled: the scroll-time hover-suppression rule put `pointer-events: none` on the scroll container itself, making the whole subtree transparent to hit-testing, so every wheel event after the first scrolled notch targeted the `overflow: hidden` ancestors and scrolled nothing until the 250 ms idle timer cleared the class. The rule now targets the container's children (the original intent of the technique), keeping the scroller wheel-reachable while still suppressing hover recalculations during scroll.

- **Reopened workflows showed execution blocks in a different order than live** (`src-tauri/src/models/chat_block.rs`, `src-tauri/src/commands/message.rs`) -- the replay merge sorted blocks by DB insert timestamps, but tool executions and reasoning steps are batch-inserted in two families at run completion, so reloading a workflow showed all tools first and all reasoning after, instead of the interleaved live order. Replay now sorts by the shared stream sequence (timestamps only break ties), each sub-agent's tool/reasoning families shift into the global sequence space with a single common offset (separate offsets destroyed their interleaving), and the sub-agent summary lands right after its internal blocks -- matching the live stream where the completion event fires last. Existing workflows benefit without migration since the order is recomputed on every load.

- **Onboarding skip button could inherit the off-screen skip-link positioning** (`OnboardingModal.svelte`) -- the footer "skip" button shared the `.skip-link` class with the global accessibility skip-to-content rules, which position that class absolute off-screen until focused; the button class is renamed so the global rules can never leak onto it. The wizard dialog also referenced a non-existent element id as its accessible name and now carries an explicit localized `aria-label`.

- **The skill enable switch flipped the wrong row** (`src-tauri/src/commands/skill.rs`, `src-tauri/src/models/skill.rs`, `src/lib/stores/skills.ts`) -- toggling a skill on or off was treated as a content revision (bumping `updated_at` and snapshotting a version), and since the skill list reloads ordered by `updated_at` descending, the toggled skill jumped to the top and the keyed `{#each}` moved the clicked switch onto a different row -- the stored value was correct, only the reordering was wrong. The backend now gates the snapshot and timestamp bump on whether the update actually touches skill content, and the frontend toggles optimistically in place (reverting on IPC failure).
...
- **French locale was missing diacritics across the app** (`src/messages/fr.json`) -- roughly a thousand words were written without their required accents (modele, execution, donnees, selectionnez, parametres, succes, ...), with accented and unaccented spellings coexisting in the same file. Every value is restored to correct French orthography, with ambiguous forms (past participle vs present tense, "a" vs "à") resolved from their sentence context; interpolation placeholders (`{operation}`, `{count}`, ...) and the `$schema` header are untouched, and EN/FR key parity is preserved.
- **English import-export summary showed raw ICU plural syntax** (`src/messages/en.json`, `MCPFieldEditor.svelte`) -- the i18n engine only performs simple `{var}` substitution, so the cleared-variables counter literally displayed `{count, plural, one {variable} other {variables}}`; the string now uses the same neutral "(s)" convention as the French locale, and an unconsumed sibling key carrying the same broken syntax is removed from both locales.
