...
export interface SubAgentExecution {
	 ...
	 * (`compute_sub_agent_cost`). Persisted in `sub_agent_execution.cost_usd`.
	 */
	cost_usd?: number;
	/**
	 * Cached prompt tokens (cache reads) reported by the sub-agent's
	 * provider. `null` for legacy rows / non-caching providers.
	 */
	cached_tokens?: number | null;
	/** Cache-write prompt tokens. Same contract as `cached_tokens`. */
	cache_write_tokens?: number | null;
	/** Thinking/reasoning tokens consumed (reasoning models only). */
